...
{
   /// <summary>
   /// Flips Maybe and Task: instead of having Maybe&lt;Task&lt;T&gt;&gt; (as in case of Select) we get 
   /// Task&lt;Maybe&lt;T&gt;&gt; and have possibility to await on it.
   /// </summary>
   ...
   public static async Task<TResult> SelectOrElse<T, TResult>(
      this Task<Maybe<T>> @this,
      Func<T, Task<TResult>> fn,
      Func<Task<TResult>> @else)
   {
      var res = await @this;
      return await res.SelectOrElse(fn, @else);
   }

   public static async Task SelectOrElse<T, TResult>(
      this Task<Maybe<T>> @this,
      Func<T, Task> fn,
      Func<Task> @else)
   {
      var res = await @this;
      await res.SelectOrElse(fn, @else);
   }
