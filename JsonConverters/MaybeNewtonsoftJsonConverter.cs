...
namespace Sero.Functional.JsonConverters
{
	internal class MaybeNewtonsoftJsonConverter : JsonConverter
	{
		...
		private class TypeConverter
		{
			...
			public TypeConverter(Type maybeConverterType)
			{
				...
				converter,
				maybeConverterType.GetMethod(nameof(MaybeNewtonsoftJsonConverter.WriteJson)));
				...
				_readJsonDelegate = (Func<JsonReader, JsonSerializer, object>)Delegate.CreateDelegate(
					...
					converter,
					maybeConverterType.GetMethod(nameof(MaybeNewtonsoftJsonConverter.ReadJson)));
